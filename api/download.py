...
def download(download_id):
    ...
    for ext in ['zip', 'mp3', 'wav', 'ogg', 'aac', 'flac', 'm4a', 'mp4', 'webm', 'avi', 'mov', 'gif', 'jpg', 'png', 'webp', 'svg', 'bmp', 'ico', 'txt', 'pdf', 'csv', 'json', 'html', 'stl', 'obj']:
        if os.path.exists(os.path.join(DOWNLOAD_DIR, f'{download_id}.{ext}')):
            download_path = os.path.join(DOWNLOAD_DIR, f'{download_id}.{ext}')
            break
    
    if not os.path.exists(download_path) or not os.path.exists(meta_path):
        return 'Ссылка недействительна или уже использована', 404
    
    with open(meta_path, 'r') as f:
        meta = json.load(f)
    
    if meta.get('downloaded', False):
        return 'Ссылка уже использована', 403
